<template>
    ...
    <el-card>
        <el-table :data="returnList">
            ...
            <el-table-column prop="type" label="表单类型" :formatter="judgeFormKind"/>
            <el-table-column label="操作">
                <template #default="scope">
                    <el-button size="small" plain type="primary" @click="showFormURL(scope.row.url)">表单链接</el-button>
                    <el-button size="small" plain type="success" @click="viewFormResult(scope.row.id)">查看填写结果</el-button>
                    <el-button size="small" plain type="info" @click="viewFormTemplate(scope.row.tid)">查看表单模板</el-button>
                </template>
            </el-table-column>
        </el-table>
    </el-card>
...
export default{
    ...
    methods:{
        ...
        showFormURL(url){
            ElMessageBox.alert('表单的URL：'+url, '表单链接', { confirmButtonText: '确认' })
            .then(() => {
